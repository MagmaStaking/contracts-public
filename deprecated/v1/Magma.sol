...
import {IGVault} from "./IGVault.sol";
...
/**
 * @dev v1 Magma, IGVault and MagmaErrorsModule files changed in v2, the rest of the files did not, so it is not
 *  neccesary to import the other files from deprecated/v1. gVault did change but we are going to update through a
 *  function pointing to a new contract address, so we just need to update IGVault in Magma v2. CoreVault will stay the
 *  same
