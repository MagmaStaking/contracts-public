...
import {ErrZeroAssets, ErrDelegateFailed, ErrUndelegateFailed, ErrCompleteUndelegationFailed} from "./MagmaErrorsModule.sol";
...
abstract contract MagmaDelegationModule {
    address internal constant STAKING_PRECOMPILE =
    ...
    function _delegate(uint64 valId, uint256 amount) internal {
        ...
        (bool ok, ) = STAKING_PRECOMPILE.call(
            abi.encodeWithSelector(SEL_DELEGATE, valId, amount)
        );
    ...
    function _undelegate(
        ...
        if (amount == 0) revert ErrZeroAssets();
        (bool ok, ) = STAKING_PRECOMPILE.call(
            abi.encodeWithSelector(SEL_UNDELEGATE, valId, amount, withdrawalId)
        );
    ...
    function _withdraw(uint64 valId, uint8 withdrawalId) internal {
        (bool ok, ) = STAKING_PRECOMPILE.call(
            abi.encodeWithSelector(SEL_WITHDRAW, valId, withdrawalId)
        );
    ...
    function _compound(uint64 valId) internal {
        (bool ok, ) = STAKING_PRECOMPILE.call(
            abi.encodeWithSelector(SEL_COMPOUND, valId)
        );
        if (!ok) revert ErrDelegateFailed();
    }
    ...
    function _claim(uint64 valId) internal {
        (bool ok, ) = STAKING_PRECOMPILE.call(
            abi.encodeWithSelector(SEL_CLAIM_REWARDS, valId)
        );
        if (!ok) revert ErrDelegateFailed();
    }
    ...
    {
        (bool ok, bytes memory ret) = STAKING_PRECOMPILE.staticcall(
    ...
    function _getDelegatorStake(
        ...
        (bool ok, bytes memory ret) = STAKING_PRECOMPILE.staticcall(
            abi.encodeWithSelector(SEL_GET_DELEGATOR, valId, delegator)
        );
