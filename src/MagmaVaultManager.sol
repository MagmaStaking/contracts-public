...
abstract contract MagmaVaultManager is MagmaRoleManagementModule {
    ...
    function undelegate(uint256 amount) external {
        (bool success,) = coreVault.call(abi.encodeWithSignature("undelegate(uint256)", amount));
    ...
    function undelegateFromValidator(uint64 valId, uint256 amount) external {
        if (gVault == address(0)) revert ErrGVaultNotSet();
        (bool success,) =
            gVault.call(abi.encodeWithSignature("undelegate(address,uint64,uint256)", msg.sender, valId, amount));
    ...
    function completeUndelegation(uint64 valId, uint8 withdrawalId) external {
        (bool success,) =
            coreVault.call(abi.encodeWithSignature("completeUndelegation(uint64,uint8)", valId, withdrawalId));
    ...
    function completeUndelegationFromValidator(uint256 unbondingIndex) external {
        if (gVault == address(0)) revert ErrGVaultNotSet();
        (bool success,) = gVault.call(abi.encodeWithSignature("completeUndelegation(uint256)", unbondingIndex));
        if (!success) revert ErrGVCompleteFailed();
    }

    function _undelegate(uint256 assets) internal override {
        (bool ok1,) = coreVault.call(abi.encodeWithSignature("undelegate(uint256)", assets));
    ...
    function _undelegateFromValidator(uint64 valId, uint256 assets) internal override {
        if (gVault == address(0)) revert ErrGVaultNotSet();
        (bool ok,) =
            gVault.call(abi.encodeWithSignature("undelegate(address,uint64,uint256)", msg.sender, valId, assets));
        if (!ok) revert ErrGVUndelegateFailed();
    }

    function _completeUndelegationFromGVault(uint256 /*assets*/ ) internal override {
        if (gVault == address(0)) revert ErrGVaultNotSet();
        (bool ok,) = gVault.call(abi.encodeWithSignature("completeUndelegation(uint256)", 0));
    ...
    function rebalanceVaults() external {
        ...
        uint256 coreTotal = 0;
        (bool ok, bytes memory data) = coreVault.staticcall(abi.encodeWithSignature("getTotalDelegated()"));
        if (ok && data.length > 0) {
            coreTotal = abi.decode(data, (uint256));
        }
        if (coreTotal == 0) {
            (bool s1,) = gVault.call(abi.encodeWithSignature("adminInitiateRebalanceBps(uint16)", 3000));
            if (!s1) revert ErrRebalanceInitiateFailed();
        }

        (bool s2,) = gVault.call(abi.encodeWithSignature("adminCompleteAndForward()"));
